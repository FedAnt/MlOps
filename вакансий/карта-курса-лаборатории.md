...
**Приоритет ближайших блоков:** фаза 5 (минимум F) → **фаза 5b Terraform** → добить Sentry/H → 6–8.

---

## На завтра / ближайшая сессия

**Старт:** «продолжаем **Фаза 5 трек F** — Prometheus» ([`программа-обучения-sentry-prometheus-statsd.md`](./программа-обучения-sentry-prometheus-statsd.md)).

Не повторять: Airflow 1–5, dbt A–D, §6.6, §0 observability.

| Шаг | Действие | Критерий «готово» |
| --- | --- | --- |
| 1 | F.1–F.2 — pull-модель, labels, scrape | Устно + набросок scrape |
| 2 | F.4 — Prometheus в k3s; targets UP | Helm/values + UI/API |
| 3 | F.3 — PromQL (+ опц. Grafana) | Один запрос data-SLI или health |
...
1. **Фаза 5** — минимум Prometheus/Grafana ([`программа-обучения-sentry-prometheus-statsd.md`](./программа-обучения-sentry-prometheus-statsd.md) трек F).
2. **Фаза 5b** — Terraform/OpenTofu MVP ([`программа-обучения-terraform-opentofu.md`](./программа-обучения-terraform-opentofu.md)).
3. Добить StatsD/Sentry/H по фазе 5 (не блокировать оффер вечным углублением).
4. Фазы 6–8 (DR, безопасность, упаковка под интервью).
...
| **5b** | Terraform / OpenTofu MVP (+ CI validate/plan) | ⏳ | [`программа-обучения-terraform-opentofu.md`](./программа-обучения-terraform-opentofu.md) — после трека F |
...
| [Prometheus / StatsD / Sentry](программа-обучения-sentry-prometheus-statsd.md) | **8 / ~20** | §0 ✅ · E ⏳ · **F** (F.1–F.4 ✅; F.5 / Grafana опц. ⏳) · G–H ⏳ |
| [Terraform / OpenTofu](программа-обучения-terraform-opentofu.md) | **0 / 6** | T0–T5 ⏳ (старт после F) |
...
| 5b | Не начато (после F) | ~1–2 нед. вечерами |
