...
**Старт:** «продолжаем **Фаза 5** — observability» (§0 → трек **F Prometheus** в [`программа-обучения-sentry-prometheus-statsd.md`](./программа-обучения-sentry-prometheus-statsd.md)).

Не повторять: Airflow 1–5, dbt A–D, §6.6.

| Шаг | Действие | Критерий «готово» |
| --- | --- | --- |
| 1 | §0 — роли метрик / логов / ошибок в data-path | Устно: Prometheus vs Sentry vs логи |
| 2 | Трек **F** — Prometheus baseline (+ Grafana по чеклисту) | Targets UP; один PromQL / дашборд |
| 3 | После F: **фаза 5b** Terraform MVP | [`программа-обучения-terraform-opentofu.md`](./программа-обучения-terraform-opentofu.md) T0–T5 |
